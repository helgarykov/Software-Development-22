...
namespace Galaga {
    ...
    public class Enemy : Entity, IGameEventProcessor {
        ...
        public Enemy(DynamicShape shape, IBaseImage greenImage, IBaseImage redImage) : base(shape, greenImage) {
            ...
            redImages = redImage;
            movementspeed = 1.0f + ((float)Game.roundNO)/10f;
            init_X = shape.Position.X;
            init_Y = shape.Position.Y;
            Game.eventBus.Subscribe(GameEventType.ControlEvent, this); // subscribes an enemy object to GameEventBus
        }
        ...
        private void beenHit(int damage) {
            ...
            if (hitpoints < 3 ) {
                ...
                DeleteEntity();
                Game.AddExplosion(entity.Shape.Position, entity.Shape.Extent);
                Game.eventBus.RegisterEvent (new GameEvent { 
